document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!localStorage.getItem("usuario")) {
        fetch('/auth/usuario') // Aquí se corrige la ruta
            .then(res => {
            ...
            .then(user => {
                if (user.autenticado) {
                    localStorage.setItem("usuario", JSON.stringify(user.usuario));  // Solo guardamos id y nombre
                    location.reload(); // Recargamos para aplicar los cambios en la interfaz
                }
            })
            .catch(() => {
